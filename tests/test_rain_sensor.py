# tests/test_rain_sensor.py
import unittest
from unittest.mock import AsyncMock, patch
from pychonet.EchonetInstance import EchonetInstance
from pychonet.RainSensor import RainSensor
...
class MockECHONETAPIClient:
    def __init__(self):
        self._state = {
            "192.168.1.7": {
                "instances": {
                    0x00: {  # EoJGC (Class Group Code) for Rain Sensor
                        0x13: {  # EoJCC (Class Code) for Rain Sensor
                            0x01: {  # Instance code
                                0x80: b'\x30',  # Operation status: 0x30 = 'ON'
                                0xB0: b'\x35',  # Detection threshold level: 0x35 = 'level-5'
                                0xB1: b'\x42',  # Rain detection status: 0x42 = 'not found'
...
class TestRainSensor(unittest.IsolatedAsyncioTestCase):
    async def test_getOperationStatus(self):
        api_connector = MockECHONETAPIClient()
        sensor = RainSensor("192.168.1.7", api_connector)
    ...
    async def test_getRainDetectionStatus(self):
        api_connector = MockECHONETAPIClient()
        sensor = RainSensor("192.168.1.7", api_connector)

        status = await sensor.getRainDetectionStatus()
        self.assertEqual(status, b'\x42')  # 'not found'

        result = await sensor.update(0xB1)
        expected = 'Rain detection status not found'
